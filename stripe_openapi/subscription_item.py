...
class Subscription_Item(Base):
    ...
    plan = Column(Plan)
    price = Column(Price)
    quantity = Column(Integer, comment='The [quantity](https://stripe.com/docs/subscriptions/quantities) of the plan to which the customer should be subscribed', nullable=True)
    subscription = Column(String, comment='The `subscription` this `subscription_item` belongs to')
    tax_rates = Column(list, comment='The tax rates which apply to this `subscription_item`. When set, the `default_tax_rates` on the subscription do not apply to this `subscription_item`', nullable=True)
