...
class Funding_Instructions_Bank_Transfer_Iban_Record(Base):
    ...
    country = Column(String, comment='Two-letter country code ([ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2))')
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Funding_Instructions_Bank_Transfer_Iban_Record(account_holder_name={account_holder_name!r}, bic={bic!r}, country={country!r}, iban={iban!r})'.format(account_holder_name=self.account_holder_name, bic=self.bic, country=self.country, iban=self.iban)
