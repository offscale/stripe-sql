...
class Terminal_Reader_Reader_Resource_Set_Reader_Display_Action(Base):
    ...
    cart = Column(TerminalReaderReaderResourceCart, comment='Cart object to be displayed by the reader', nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Terminal_Reader_Reader_Resource_Set_Reader_Display_Action(cart={cart!r}, type={type!r}, id={id!r})'.format(cart=self.cart, type=self.type, id=self.id)
