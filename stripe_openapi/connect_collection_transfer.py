...
class Connect_Collection_Transfer(Base):
    ...
    destination = Column(Account, comment='ID of the account that funds are being collected for')
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Connect_Collection_Transfer(amount={amount!r}, currency={currency!r}, destination={destination!r}, id={id!r}, livemode={livemode!r}, object={object!r})'.format(amount=self.amount, currency=self.currency, destination=self.destination, id=self.id, livemode=self.livemode, object=self.object)
