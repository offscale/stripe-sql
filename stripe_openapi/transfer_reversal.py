...
class Transfer_Reversal(Base):
    ...
    destination_payment_refund = Column(Refund, comment='Linked payment refund for the transfer reversal', nullable=True)
    ...
    source_refund = Column(Refund, comment='ID of the refund responsible for the transfer reversal', nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Transfer_Reversal(amount={amount!r}, balance_transaction={balance_transaction!r}, created={created!r}, currency={currency!r}, destination_payment_refund={destination_payment_refund!r}, id={id!r}, metadata={metadata!r}, object={object!r}, source_refund={source_refund!r}, transfer={transfer!r})'.format(amount=self.amount, balance_transaction=self.balance_transaction, created=self.created, currency=self.currency, destination_payment_refund=self.destination_payment_refund, id=self.id, metadata=self.metadata, object=self.object, source_refund=self.source_refund, transfer=self.transfer)
