...
class Payment_Intent_Next_Action_Wechat_Pay_Redirect_To_Android_App(Base):
    ...
    app_id = Column(String, comment='app_id is the APP ID registered on WeChat open platform')
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Payment_Intent_Next_Action_Wechat_Pay_Redirect_To_Android_App(app_id={app_id!r}, nonce_str={nonce_str!r}, package={package!r}, partner_id={partner_id!r}, prepay_id={prepay_id!r}, sign={sign!r}, timestamp={timestamp!r}, id={id!r})'.format(app_id=self.app_id, nonce_str=self.nonce_str, package=self.package, partner_id=self.partner_id, prepay_id=self.prepay_id, sign=self.sign, timestamp=self.timestamp, id=self.id)
