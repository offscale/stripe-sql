...
class Payment_Method_Options_Afterpay_Clearpay(Base):
    ...
    reference = Column(String, comment='Order identifier shown to the customer in Afterpay’s online portal. We recommend using a value that helps you answer any questions a customer might have about\nthe payment. The identifier is limited to 128 characters and may contain only letters, digits, underscores, backslashes and dashes', nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Payment_Method_Options_Afterpay_Clearpay(capture_method={capture_method!r}, reference={reference!r}, setup_future_usage={setup_future_usage!r}, id={id!r})'.format(capture_method=self.capture_method, reference=self.reference, setup_future_usage=self.setup_future_usage, id=self.id)
