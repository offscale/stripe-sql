...
class LineItem(Base):
    ...
    subscription_item = Column(
        String,
        comment="The subscription item that generated this invoice item. Left empty if the line item is not an explicit result of a subscription",
        nullable=True,
    )
    tax_amounts = Column(
        list,
    ...
    tax_rates = Column(
        list, comment="The tax rates which apply to the line item", nullable=True
    )
    type = Column(
        String,
