...
class Balance_Transaction(Base):
    ...
    available_on = Column(Integer, comment="The date the transaction's net funds will become available in the Stripe balance")
    ...
    reporting_category = Column(String, comment='[Learn more](https://stripe.com/docs/reports/reporting-categories) about how reporting categories can help you understand balance transactions from an accounting perspective')
    source = Column(BalanceTransactionSource, comment='The Stripe object to which this transaction is related', nullable=True)
    status = Column(String, comment="If the transaction's net funds are available in the Stripe balance yet. Either `available` or `pending`")
    type = Column(String, comment='Transaction type: `adjustment`, `advance`, `advance_funding`, `anticipation_repayment`, `application_fee`, `application_fee_refund`, `charge`, `connect_collection_transfer`, `contribution`, `issuing_authorization_hold`, `issuing_authorization_release`, `issuing_dispute`, `issuing_transaction`, `payment`, `payment_failure_refund`, `payment_refund`, `payout`, `payout_cancel`, `payout_failure`, `refund`, `refund_failure`, `reserve_transaction`, `reserved_funds`, `stripe_fee`, `stripe_fx_fee`, `tax_fee`, `topup`, `topup_reversal`, `transfer`, `transfer_cancel`, `transfer_failure`, or `transfer_refund`. [Learn more](https://stripe.com/docs/reports/balance-transaction-types) about balance transaction types and what they represent. If you are looking to classify transactions for accounting purposes, you might want to consider `reporting_category` instead')
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Balance_Transaction(amount={amount!r}, available_on={available_on!r}, created={created!r}, currency={currency!r}, description={description!r}, exchange_rate={exchange_rate!r}, fee={fee!r}, fee_details={fee_details!r}, id={id!r}, net={net!r}, object={object!r}, reporting_category={reporting_category!r}, source={source!r}, status={status!r}, type={type!r})'.format(amount=self.amount, available_on=self.available_on, created=self.created, currency=self.currency, description=self.description, exchange_rate=self.exchange_rate, fee=self.fee, fee_details=self.fee_details, id=self.id, net=self.net, object=self.object, reporting_category=self.reporting_category, source=self.source, status=self.status, type=self.type)
