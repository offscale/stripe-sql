...
class Setup_Intent_Payment_Method_Options_Mandate_Options_Blik(Base):
    ...
    expires_after = Column(Integer, comment='Date at which the mandate expires', nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Setup_Intent_Payment_Method_Options_Mandate_Options_Blik(expires_after={expires_after!r}, off_session={off_session!r}, type={type!r}, id={id!r})'.format(expires_after=self.expires_after, off_session=self.off_session, type=self.type, id=self.id)
