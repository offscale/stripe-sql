...
class Setup_Intent(Base):
    ...
    application = Column(
        application,
        comment="[[FK(application)]] ID of the Connect application that created the SetupIntent",
        nullable=True,
    )
    ...
    customer = Column(
        string | customer,
        comment="[[FK(deleted_customer)]] ID of the Customer this SetupIntent belongs to, if one exists.\n\nIf present, the SetupIntent's payment method will be attached to the Customer on successful setup. Payment methods attached to other Customers cannot be used with this SetupIntent",
        nullable=True,
    )
    description = Column(
        String,
    ...
    last_setup_error = Column(
        api_errors,
        comment="[[FK(api_errors)]] The error encountered in the previous SetupIntent confirmation",
        nullable=True,
    )
    latest_attempt = Column(
        setup_attempt,
        comment="[[FK(setup_attempt)]] The most recent SetupAttempt for this SetupIntent",
        nullable=True,
    )
    livemode = Column(
        Boolean,
    ...
    mandate = Column(
        mandate,
        comment="[[FK(mandate)]] ID of the multi use Mandate generated by the SetupIntent",
        nullable=True,
    )
    metadata = Column(
        JSON,
    ...
    next_action = Column(
        ...
        comment="[[FK(setup_intent_next_action)]] If present, this property tells you what actions you need to take in order for your customer to continue payment setup",
        nullable=True,
    )
    object = Column(
        String,
    ...
    on_behalf_of = Column(
        account,
        comment="[[FK(account)]] The account (if any) for which the setup is intended",
        nullable=True,
    )
    payment_method = Column(
        payment_method,
        comment="[[FK(payment_method)]] ID of the payment method used with this SetupIntent",
        nullable=True,
    )
    payment_method_options = Column(
        setup_intent_payment_method_options,
        comment="[[FK(setup_intent_payment_method_options)]] Payment-method-specific configuration for this SetupIntent",
        nullable=True,
    )
    ...
    single_use_mandate = Column(
        mandate,
        comment="[[FK(mandate)]] ID of the single_use Mandate generated by the SetupIntent",
        nullable=True,
    )
    status = Column(
        String,
