...
class Account_Future_Requirements(Base):
    ...
    currently_due = Column(
        ARRAY(String),
        comment="Fields that need to be collected to keep the account enabled. If not collected by `future_requirements[current_deadline]`, these fields will transition to the main `requirements` hash",
        nullable=True,
    )
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return "Account_Future_Requirements(alternatives={alternatives!r}, current_deadline={current_deadline!r}, currently_due={currently_due!r}, disabled_reason={disabled_reason!r}, errors={errors!r}, eventually_due={eventually_due!r}, past_due={past_due!r}, pending_verification={pending_verification!r}, id={id!r})".format(
