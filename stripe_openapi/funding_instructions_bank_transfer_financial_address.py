...
class Funding_Instructions_Bank_Transfer_Financial_Address(Base):
    ...
    supported_networks = Column(ARRAY(String), comment='The payment networks supported by this FinancialAddress', nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Funding_Instructions_Bank_Transfer_Financial_Address(iban={iban!r}, sort_code={sort_code!r}, spei={spei!r}, supported_networks={supported_networks!r}, type={type!r}, zengin={zengin!r}, id={id!r})'.format(iban=self.iban, sort_code=self.sort_code, spei=self.spei, supported_networks=self.supported_networks, type=self.type, zengin=self.zengin, id=self.id)
