...
class Treasury_Shared_Resource_Billing_Details(Base):
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Treasury_Shared_Resource_Billing_Details(address={address!r}, email={email!r}, name={name!r}, id={id!r})'.format(address=self.address, email=self.email, name=self.name, id=self.id)
