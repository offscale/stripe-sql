...
class Subscription_Transfer_Data(Base):
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Subscription_Transfer_Data(amount_percent={amount_percent!r}, destination={destination!r}, id={id!r})'.format(amount_percent=self.amount_percent, destination=self.destination, id=self.id)
