...
class Financial_Reporting_Finance_Report_Run_Run_Parameters(Base):
    ...
    interval_end = Column(
        Integer,
        comment="Ending timestamp of data to be included in the report run (exclusive)",
        nullable=True,
    )
    interval_start = Column(
        Integer,
        comment="Starting timestamp of data to be included in the report run",
        nullable=True,
    )
    payout = Column(
        String, comment="Payout ID by which to filter the report run", nullable=True
    )
