...
class Checkout_Customer_Balance_Payment_Method_Options(Base):
    ...
    bank_transfer = Column(CheckoutCustomerBalanceBankTransferPaymentMethodOptions, nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Checkout_Customer_Balance_Payment_Method_Options(bank_transfer={bank_transfer!r}, funding_type={funding_type!r}, setup_future_usage={setup_future_usage!r}, id={id!r})'.format(bank_transfer=self.bank_transfer, funding_type=self.funding_type, setup_future_usage=self.setup_future_usage, id=self.id)
