...
class File_Link(Base):
    ...
    expires_at = Column(Integer, comment='Time at which the link expires', nullable=True)
    ...
    url = Column(String, comment='The publicly accessible URL to download the file', nullable=True)
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'File_Link(created={created!r}, expired={expired!r}, expires_at={expires_at!r}, file={file!r}, id={id!r}, livemode={livemode!r}, metadata={metadata!r}, object={object!r}, url={url!r})'.format(created=self.created, expired=self.expired, expires_at=self.expires_at, file=self.file, id=self.id, livemode=self.livemode, metadata=self.metadata, object=self.object, url=self.url)
__all__ = ['file_link']