...
class Source_Transaction(Base):
    ...
    amount = Column(Integer, comment='A positive integer in the smallest currency unit (that is, 100 cents for $1.00, or 1 for ¥1, Japanese Yen being a zero-decimal currency) representing the amount your customer has pushed to the receiver')
    ...
    sepa_credit_transfer = Column(SourceTransactionSepaCreditTransferData, nullable=True)
    source = Column(String, comment='The ID of the source this transaction is attached to')
    status = Column(String, comment='The status of the transaction, one of `succeeded`, `pending`, or `failed`')
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Source_Transaction(ach_credit_transfer={ach_credit_transfer!r}, amount={amount!r}, chf_credit_transfer={chf_credit_transfer!r}, created={created!r}, currency={currency!r}, gbp_credit_transfer={gbp_credit_transfer!r}, id={id!r}, livemode={livemode!r}, object={object!r}, paper_check={paper_check!r}, sepa_credit_transfer={sepa_credit_transfer!r}, source={source!r}, status={status!r}, type={type!r})'.format(ach_credit_transfer=self.ach_credit_transfer, amount=self.amount, chf_credit_transfer=self.chf_credit_transfer, created=self.created, currency=self.currency, gbp_credit_transfer=self.gbp_credit_transfer, id=self.id, livemode=self.livemode, object=self.object, paper_check=self.paper_check, sepa_credit_transfer=self.sepa_credit_transfer, source=self.source, status=self.status, type=self.type)
