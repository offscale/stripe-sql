...
class Package_Dimensions(Base):
    ...
    def __repr__(self):
        ...
        :rtype: ```str```
        """
        return 'Package_Dimensions(height={height!r}, length={length!r}, weight={weight!r}, width={width!r}, id={id!r})'.format(height=self.height, length=self.length, weight=self.weight, width=self.width, id=self.id)
